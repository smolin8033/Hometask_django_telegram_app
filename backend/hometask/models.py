...
class Hometask(models.Model):
    ...
    coursebook = models.CharField(
        max_length=50, verbose_name="Название учебника", blank=True
    )
    exercises = models.CharField(
        max_length=50, verbose_name="Упражнения для выполнения", blank=True
    )
    url = models.URLField(
        max_length=200,
...
class HometaskImage(models.Model):
    ...
    hometask = models.ForeignKey(
        Hometask, related_name="images", on_delete=models.CASCADE
    )
...
class HometaskFile(models.Model):
    ...
    hometask = models.ForeignKey(
        Hometask, related_name="files", on_delete=models.CASCADE
    )
